...
public class AuthenticationRestController {
    ...
    @PostMapping("/signup")
    public ResponseEntity<JwtAuthResponse> signUp(@Valid @RequestBody UserSignUpRequest request) throws UserAuthNameOrEmailExisten {
        log.info("Registrando usuario: {}", request);
        return ResponseEntity.ok(authService.singUp(request));
    }

    @PostMapping("/signin")
    public ResponseEntity<JwtAuthResponse> signIn(@Valid @RequestBody UserSignInRequest request) throws AuthSingInInvalid {
